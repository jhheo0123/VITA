...
def pad_batch(batch):
    ...
    # Count the number of diseases, procedures, and medications for each sequence, along with their corresponding min/max values
    # Also calculate the intersection and difference between the disease of each sequence and the previous sequence's disease
    d_length_matrix = []
    p_length_matrix = []
    m_length_matrix = []
    d_max_num = 0
    ...
    for data in batch:
        ...
        for idx, seq in enumerate(data):
            ...
            if idx==0:
                # For the first sequence, both the intersection and difference are empty.
                d_dec_list_buf.append([])
                d_stay_list_buf.append([])
            else:
                # Calculate the set difference and intersection.
                cur_d = set(seq[0])
    ...
    # Generate data for disease, procedure, and medication.
    ...
    for b_id, data in enumerate(batch):
        for s_id, adm in enumerate(data):
            # Sort the admission data according to disease, procedure, medication.
...
def pad_batch_v2_train(batch):
    ...
    # Calculate the counts and corresponding min/max values for diseases, procedures, and medications in each sequence
    # Additionally, compute the intersection and difference between the diseases of each sequence and the previous sequence's diseases
    d_length_matrix = []
    p_length_matrix = []
    m_length_matrix = []
    d_max_num = 0
    ...
    d_max_num = 39 
    p_max_num = 32
    m_max_num  =56   # 53 # mimic-iv change 56
    ...
    # Generate data for disease, procedure, and medication separately.
    disease_tensor = torch.full((batch_size, max_seq, d_max_num), -1)
    procedure_tensor = torch.full((batch_size, max_seq, p_max_num), -1)
    medication_tensor = torch.full((batch_size, max_seq, m_max_num), 0)

    # Concatenate them into a batch of data
    for b_id, data in enumerate(batch):
        for s_id, adm in enumerate(data):
            # Sort the admission data according to disease, procedure, medication.
            disease_tensor[b_id, s_id, :len(adm[0])] = torch.tensor(adm[0])
...
def pad_batch_v2_eval(batch):
    ...
    # Count the number of diseases, procedures, and medications for each sequence, along with their corresponding min/max values
    # Also calculate the intersection and difference between the diseases of each sequence and the previous sequence's diseases
    d_length_matrix = []
    p_length_matrix = []
    m_length_matrix = []
    d_max_num = 0
