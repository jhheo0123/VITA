...
class Beam(object):
    ...
    def __init__(self, size, PAD_TOKEN, BOS_TOKEN, EOS_TOKEN, ddi_adj, device):
        ...
        self.device = device
        self.size = size
        self.done = False   # Indicates whether the beam search process has been completed

        self.beam_status = [False] * size   # Used to record whether each beam is already in the EOS state

        self.tt = torch.cuda if device.type=='cuda' else torch

        # The score of each generated result, initially there are beam_size zeros
