...
""" GCN for make medication embeddings """
class GCN(nn.Module):
...
class Leap(nn.Module):
    ...
    def forward(self, input, max_len=20):
        ...
        if self.training:
            ...
            # Calculate for each known medication separately, somewhat similar to the teacher-forcing approach
            for med_code in [self.SOS_TOKEN] + input[2]:
                ...
                if hidden_state is None:    # Utilize the hidden state from the previous admission.
                ...
                # (1, len)   Calculate the attention weights of that medication for all medical conditions.
                attn_weight = F.softmax(self.attn(combined_input).t(), dim=-1)
                input_embedding = attn_weight.mm(
                    input_embedding)  # (1, dim)    # Compute the weighted sum.
                ...
                # (1, med_num) Represents the logits for each medication at the current position.
                output_logits.append(self.output(F.relu(hidden_state)))
            return torch.cat(output_logits, dim=0)

        else:
        # During testing, we cannot use input[2], which is medication information
        # Control the maximum length (adjustable based on data range)
            for di in range(max_len):
                ...
                # Data is directly taken from the dataset, and here we directly obtain the most probable logits for the current position.
...
class Leap_batch(nn.Module):
    def __init__(self, voc_size, emb_dim=64, device=torch.device('cpu:0')):
        ...
        self.med_embedding = nn.Sequential(
            # Add padding_idx to represent the zero vector.
        ...
        # Used for generating medication sequences.
        self.dec_gru = nn.GRU(emb_dim*2, emb_dim, batch_first=True)
    ...
    def forward(self, diseases, procedures, medications, d_mask_matrix, m_mask_matrix, seq_length, max_len=20):
        ...
        #last_seq_medication，Represents the corresponding medication from the previous sequence for each seq. For the first instance, since there's no previous medication, it's padded with 0 (or any other filler, as it won't be used).
        ...
        # The m_mask_matrix matrix also needs to be shifted to the right.
        last_m_mask = torch.full((batch_size, 1, max_med_num), -1e9).to(device) # Use a larger negative value here to prevent the softmax operation from distributing probabilities too widely.
        last_m_mask = torch.cat([last_m_mask, m_mask_matrix[:, :-1, :]], dim=1)

        last_seq_medication_emb = self.med_embedding(last_seq_medication)
        # Encode last_visit, using the encoding result of last_seq_medication
        ...
        # Similarly, concatenate a `last_medication` for sequence generation, noting that the dimension is increased by one.
        ...
        # Begin iterating to generate results for each position.
        for i in range(loop_size):
            if self.training:
                dec_input = self.med_embedding(last_medication[:,:,i])  # (batch, seq, emb_dim)  Retrieve the embedding of the previous drug.
            else:
                if i==0: 
            ...
            # hidden_state (batch, seq, emb_dim)
            ...
            attn_weight=F.softmax(attn_score, dim=-1).unsqueeze(dim=2)   # (batch, seq, 1, max_diag_num) 
