...
def sequence_output_process(output_logits, filter_token):
    """Generate the final correct sequence, output_logits represents the prob at each position, and filter_token represents SOS and END"""
    pind = np.argsort(output_logits, axis=-1)[:, ::-1]  # Sort by probability in descending order at each position.

    out_list = []   # Generated results
    break_flag = False
    for i in range(len(pind)):
        # Sequentially traverse all values on pind.
        # Use break_flag to determine whether to exit the sentence generation loop.
        if break_flag:
            break
        # The results are sorted in descending order at each position.
        for j in range(pind.shape[1]):
            label = pind[i][j]
            # If SOS or END is encountered, it means the sentence is over.
            if label in filter_token:
                break_flag = True
                break
            # If an unseen value is encountered, continue generating.
            # Otherwise, look at the next drug with a higher probability.
    ...
    # Sort all drugs in out_list by probability from high to low
...
def ddi_rate_score(record, path= '../data/mimic-iii/ddi_A_final.pkl'): # '../data/mimic-iv/ddi_A_final2.pkl' 
    # ddi rate
