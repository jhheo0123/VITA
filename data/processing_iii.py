...
def med_process(med_file):
    """Read the original MIMIC data file, retaining pid, adm_id, data, and NDC, and return as a DataFrame type."""
    # Read the drug file. The NDC (National Drug Code) is stored as a category type.
    ...
    med_pd = med_pd.reset_index(drop=True)  # Reset the index and drop the original index
...
def ndc2atc4(med_pd):
    ...
    # Read the mapping from NDC to xnorm based on the ndc_rxnorm_file (this xnorm seems equivalent to the RXCUI below)
    med_pd['RXCUI'] = med_pd['NDC'].map(ndc2rxnorm)
    med_pd.dropna(inplace=True) # In reality, nothing was removed.

    rxnorm2atc = pd.read_csv(ndc2atc_file)
    rxnorm2atc = rxnorm2atc.drop(columns=['YEAR','MONTH','NDC']) # NDC is removed, directly mapping from RXCUI to ATC.
    # Remove duplicate columns based on RXCUI.
    rxnorm2atc.drop_duplicates(subset=['RXCUI'], inplace=True)

    med_pd.drop(index = med_pd[med_pd['RXCUI'].isin([''])].index, axis=0, inplace=True)  # Delete specific RXCUI.
    
    med_pd['RXCUI'] = med_pd['RXCUI'].astype('int64')   
    med_pd = med_pd.reset_index(drop=True)
    med_pd = med_pd.merge(rxnorm2atc, on=['RXCUI'])     # Merge the two tables
    med_pd.drop(columns=['NDC', 'RXCUI'], inplace=True) # Remove NDC and RXCUI, leaving only ATC4.
    med_pd = med_pd.rename(columns={'ATC4':'NDC'})      # Rename to NDC
    med_pd['NDC'] = med_pd['NDC'].map(lambda x: x[:4])  # Retain only the first four characters
    med_pd = med_pd.drop_duplicates()    
    med_pd = med_pd.reset_index(drop=True)
    return med_pd
...
def process_visit_lg2(med_pd):
    """Exclude patient data with less than two admissions"""
...
def filter_300_most_med(med_pd):
    # Sort by the number of occurrences of NDC in descending order and take the top 300
...
def combine_process(med_pd, diag_pd, pro_pd):
    """Combination of data on medication, symptoms, and proc"""
...
def create_patient_record(df, diag_voc, med_voc, pro_voc):
    """
    Save records of list type
    Each item represents a patient, and within each patient, there are multiple visits. 
    Each visit contains three arrays, which in order represent diagnosis, proc, and medication
    All stored are codes, and the specific words corresponding to them can be viewed through voc_final.pkl
    """
...
def get_ddi_matrix(records, med_voc, ddi_file):
    ...
    med_unique_word = [med_voc.idx2word[i] for i in range(med_voc_size)]    # ATC4 of all medications
    ...
    # fliter sever side effect，It also takes the form of topK
    ...
    # ddi adj, the DDI table is encoded with CID. Therefore, CID needs to be mapped to ACT code in order to record the conflict information between drugs in the dataset.
...
if __name__ == '__main__':
    ...
    # please input into your own mimic-iii folder
    # MIMIC data files, which include medications, diagnoses, and proc respectively
    med_file = 'mimic-iii/PRESCRIPTIONS.csv'
    diag_file = 'mimic-iii/DIAGNOSES_ICD.csv'
    procedure_file = 'mimic-iii/PROCEDURES_ICD.csv'

    # Medication information
    med_structure_file = 'idx2drug.pkl'   # Mapping of drugs to molecular formulas

    # drug code mapping files
    ndc2atc_file = 'ndc2atc_level4.csv'   # NDC code to ATC-4 code mapping file，Used to read from xnorm to ATC
    cid_atc = 'drug-atc.csv'              # drug（CID） to ATC code mapping file，Used to process the DDI table
    ...
    med_pd_lg2 = process_visit_lg2(med_pd).reset_index(drop=True)   # Note that this is only for patients in the med table who have had two or more admissions
    ...
    records = create_patient_record(data, diag_voc, med_voc, pro_voc)   # diag, proc, medication are stored in order
